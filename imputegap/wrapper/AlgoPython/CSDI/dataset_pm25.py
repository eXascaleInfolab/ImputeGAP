...
class PM25_Dataset(Dataset):
    def __init__(self, eval_length=36, target_dim=36, mode="train", validindex=0, ts_m=None, tr_ratio=0.7, replicat=False, seed=2021):
        ...
        if replicat:
            ...
            self.train_mean = np.nanmean(ts_m, axis=0)  # shape (D,)
            self.train_std = np.nanstd(ts_m, axis=0, ddof=0)  # shape (D,)
            ...
            n_test = int(n_samples * ts_r)

            if mode == "train":
                self.month_list = indices[:n_train]
            elif mode == "valid":
                self.month_list = indices[n_train + n_test:]
            elif mode == "test":
                self.month_list = indices[n_train:n_train + n_test]
...
def get_dataloader(batch_size, device, validindex=0, ts_m=None, seq_len=24, features=35, num_workers=0, shuffle=True):
    dataset = PM25_Dataset(mode="train", eval_length=seq_len, target_dim=features, validindex=validindex, ts_m=ts_m)
    train_loader = DataLoader(dataset, batch_size=batch_size, num_workers=num_workers, shuffle=shuffle)
    dataset_test = PM25_Dataset(mode="test", eval_length=seq_len, target_dim=features, validindex=validindex, ts_m=ts_m)
    test_loader = DataLoader(dataset_test, batch_size=batch_size, num_workers=num_workers, shuffle=False)
    dataset_valid = PM25_Dataset(mode="valid", eval_length=seq_len, target_dim=features, validindex=validindex, ts_m=ts_m)
