...
def get_dataloader(datatype, device, batch_size=8, shuffle=1, tr_ratio=0.7, ts_m=None):
    dataset = Forecasting_Dataset(datatype,mode='train', ts_m=ts_m, tr_ratio=tr_ratio)
    train_loader = DataLoader(dataset, batch_size=batch_size, shuffle=shuffle)
    valid_dataset = Forecasting_Dataset(datatype,mode='valid', ts_m=ts_m, tr_ratio=tr_ratio)
    valid_loader = DataLoader(valid_dataset, batch_size=batch_size, shuffle=0)
    test_dataset = Forecasting_Dataset(datatype,mode='test', ts_m=ts_m, tr_ratio=tr_ratio)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=0)
    imputegap_dataset = Forecasting_Dataset(datatype,mode='imputegap', ts_m=ts_m, tr_ratio=1)
    imputegap_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=0)

    scaler = torch.from_numpy(dataset.std_data).to(device).float()
    mean_scaler = torch.from_numpy(dataset.mean_data).to(device).float()

    return train_loader, valid_loader, test_loader, imputegap_loader, scaler, mean_scaler